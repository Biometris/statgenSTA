...
if (modelType == "random") {
  cat(paste("The generalized heritability is",
            round(extractSTA(x, trials = trial, what = "heritability")[[trial]][["heritability"]][trait],
                  digits = 2)))
}
@
\clearpage
...
pred <- extractSTA(x, what = predName)[[trial]][[predName]][c("genotype", trait)]
predSE <- extractSTA(x, what = paste0("se", predName))[[trial]][[1]][c("genotype", trait)]
