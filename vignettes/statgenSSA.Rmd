...
The statgenSSA package is developed as an easy-to-use package for analyzing data of plant breeding experiments with many options for plotting and reporting the results of the analyses.  
...
The first step when modeling field trial data or performing a GxE analysis with the statgenSSA package is creating an object of class `TD` (Trial Data). This object is used throughout the statgenSSA package as input for analyses.  
...
Field data from an experiment with wheat in Chile described in detail by  @Lado2013 will be used as an example throughout this vignette. The experiment was performed in two locations in Chile, Santa Rosa and Cauquenes, with two different drought regimes in 2011 and 2012 for the Santa Rosa and one trial in 2012 for Cauquenes. For 384 genotypes four traits were measured in 2011, but in 2012 only grain yield (GY) was measured and the examples will mainly focus on this trait.  
...
The meta data will be a `data.frame` with four rows, one for each trial in `wheatTD`.
```{r getMeta}
...
The trials in the boxplot can be grouped using the parameter `groupBy`. Colors can be applied to groups within trials using the parameter `colorBy`. The boxes  for the (groups of) trials can be ordered using `orderBy`. Boxes can be ordered by external vector or by the group mean.
```{r boxPlotGR}
...
After fitting a model, it is possible to run an outlier detection on the results. This outlier detection checks the residuals in the fitted model and compares them to a limit. The default limit is calculated based on the number of observations in the data. Setting a custom limit is also possible.
```{r outDet}
...
F | residF | residuals for the model with genotype as fixed component |
F | stdResF | standardized residuals for the model with genotype as fixed component |
...
F | rDfF | residual degrees of freedom for the model with genotype as fixed component |
